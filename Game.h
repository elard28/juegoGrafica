...
class Game
{
    public:
        ...
        GLvoid window_arrow(int, int, int);
        GLvoid window_idle();
...
void Game::init_game(int argc, char** argv)
{
    ...
    void * letse = (void*)this->glPaint(); 

    glutDisplayFunc( letse );
    glutReshapeFunc( (void*)this->window_redraw);
    glutKeyboardFunc(this->window_key);
    glutSpecialFunc(this->window_arrow);
    glutIdleFunc(this->window_idle);
...
GLvoid Game::window_redraw(GLsizei width, GLsizei height) {
...
GLvoid Game::window_key(unsigned char key, int x, int y) 
{
    switch (key) {
    case KEY_ESC:
        exit(0);
...
GLvoid Game::window_arrow(int key, int x, int y)
{
    switch (key) 
    {
